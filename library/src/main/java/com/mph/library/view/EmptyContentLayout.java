...
/**
 * 实现loading、error、empty、content四种状态的自由切换
 *
 * Created by：hcs on 2016/12/22 10:07
 * e_mail：aaron1539@163.com
 */
public class EmptyContentLayout extends FrameLayout {
    View loadingView;
    View errorView;
    View emptyView;
    View contentView;
    ...
    public EmptyContentLayout(Context context, AttributeSet attrs) {
        super(context, attrs, 0);
    }

    public EmptyContentLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttrs(context, attrs);
    }

    private void initAttrs(Context context, AttributeSet attr) {
        TypedArray typedArray = context.obtainStyledAttributes(attr, R.styleable.EmptyContentLayout);
        loadingLayoutId = typedArray.getResourceId(R.styleable.EmptyContentLayout_em_loadingLayoutId, RES_NONE);
        errorLayoutId = typedArray.getResourceId(R.styleable.EmptyContentLayout_em_errorLayoutId, RES_NONE);
        emptyLayoutId = typedArray.getResourceId(R.styleable.EmptyContentLayout_em_emptyLayoutId, RES_NONE);
        contentLayoutId = typedArray.getResourceId(R.styleable.EmptyContentLayout_em_contentLayoutId, RES_NONE);
        typedArray.recycle();
    }
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 4) {
            throw new IllegalStateException("EmptyContentLayout can only host 4 elements");
        } else {
    ...
    public EmptyContentLayout loadingView(View loadingView) {
        bindView(loadingView, STATE_LOADING);
        return this;
    }

    public EmptyContentLayout errorView(View errorView) {
        bindView(errorView, STATE_ERROR);
        return this;
    }

    public EmptyContentLayout emptyView(View emptyView) {
        bindView(emptyView, STATE_EMPTY);
        return this;
    }
